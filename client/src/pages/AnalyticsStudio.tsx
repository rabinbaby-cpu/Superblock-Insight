...
export default function AnalyticsStudio() {
  ...
  return (
    <AppShell breadcrumbs={["Analytics Studio"]}>
      <PageHeader eyebrow="Platform performance" title="Analytics Studio" description="Customer growth, communication volume, engagement, and revenue signals across the Superblock platform." actions={<span className="data-freshness"><span className="size-1.5 rounded-full bg-emerald-500" />Updated 2 min ago</span>} />
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.55fr_1fr]">
        <div className="panel p-4">
          ...
          <div className="mt-2 grid grid-cols-3 divide-x rounded-lg bg-muted/35 px-2 py-2.5 text-center">
            <div><div className="font-tabular text-sm font-semibold">10,482</div><div className="text-[9px] uppercase tracking-wider text-muted-foreground">DAU</div></div>
            <div><div className="font-tabular text-sm font-semibold">35,640</div><div className="text-[9px] uppercase tracking-wider text-muted-foreground">WAU</div></div>
            <div><div className="font-tabular text-sm font-semibold">79,410</div><div className="text-[9px] uppercase tracking-wider text-muted-foreground">MAU</div></div>
          </div>
        </div>
        ...
        <div className="panel p-4">
          ...
          <div className="mt-2 flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 px-3 py-2"><div><div className="text-[10px] text-muted-foreground">Fastest-growing channel</div><div className="mt-0.5 text-xs font-semibold">WhatsApp · +24.6%</div></div><span className="font-tabular text-sm font-semibold">16.2M</span></div>
        </div>
      </section>
      ...
      <section className="mt-4 grid gap-4 lg:grid-cols-3">
        <div className="panel p-4 lg:col-span-1">
          ...
          <div className="relative mx-auto h-[205px] max-w-[300px]">
            ...
            <div className="pointer-events-none absolute inset-0 grid place-items-center text-center"><div><div className="font-tabular text-xl font-semibold tracking-[-0.04em]">59.0K</div><div className="text-[9px] uppercase tracking-wider text-muted-foreground">Total</div></div></div>
          </div>
          <div className="grid grid-cols-2 gap-2">{conversationSeries.map((item) => <div key={item.name} className="flex items-center justify-between rounded-md bg-muted/35 px-2.5 py-2 text-[10px]"><span className="flex items-center gap-1.5 text-muted-foreground"><span className="size-1.5 rounded-full" style={{ background: item.color }} />{item.name}</span><span className="font-tabular font-semibold">{formatNumber(item.value)}</span></div>)}</div>
        </div>
      ...
      <section className="mt-4 grid gap-4 2xl:grid-cols-[1.35fr_1fr]">
        <div className="panel overflow-hidden">
          <div className="p-4 pb-2"><SectionHeader title="Top customers by usage" description="Ranked by total cross-channel volume" action={<Link href="/customers" className="inline-flex items-center text-[11px] font-medium text-muted-foreground hover:text-foreground">View all <ChevronRight className="size-3" /></Link>} /></div>
          <div className="overflow-x-auto"><table className="data-table min-w-[760px]"><thead><tr><th>Customer</th><th className="text-right">Messages</th><th className="text-right">Broadcasts</th><th className="text-right">Conversations</th><th className="text-right">Total usage</th><th className="text-right">MRR</th></tr></thead><tbody>{topCustomers.map((customer) => <tr key={customer.id}><td><Link href={`/customers/${customer.id}`} className="font-medium hover:underline">{customer.company}</Link><div className="font-mono text-[9px] text-muted-foreground">{customer.id}</div></td><td className="text-right font-tabular">{formatNumber(customer.usage.messages)}</td><td className="text-right font-tabular">{customer.usage.broadcasts}</td><td className="text-right font-tabular">{formatNumber(customer.usage.conversations)}</td><td className="text-right font-tabular">{formatNumber(customer.usage.messages + customer.usage.email + customer.usage.sms)}</td><td className="text-right font-tabular font-medium">{formatCurrency(customer.subscription.mrr)}</td></tr>)}</tbody></table></div>
        </div>
        <div className="panel overflow-hidden">
          <div className="p-4 pb-2"><SectionHeader title="Recent activity" description="Live workspace events" action={<span className="flex items-center gap-1 text-[9px] font-medium text-emerald-600"><span className="size-1.5 rounded-full bg-emerald-500" /> LIVE</span>} /></div>
          <div className="divide-y divide-border/60">{recentActivity.map((item) => <div key={`${item.time}-${item.customer}`} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-muted/30"><div className="w-10 font-mono text-[9px] text-muted-foreground">{item.time}</div><div className="min-w-0 flex-1"><div className="truncate text-xs font-medium">{item.activity}</div><div className="truncate text-[10px] text-muted-foreground">{item.customer} · {item.channel} · {item.volume}</div></div><StatusBadge status={item.status} dot={false} /></div>)}</div>
        </div>
      </section>
    </AppShell>
  );
...
function ChartTooltip({ active, payload, label, suffix = "" }: { active?: boolean; payload?: Array<{ name: string; value: number; color: string }>; label?: string; suffix?: string }) {
  ...
  return <div className="rounded-lg border bg-popover p-2.5 text-popover-foreground shadow-lg"><div className="mb-1.5 text-[10px] font-medium text-muted-foreground">{label}</div>{payload.map((item) => <div key={item.name} className="flex min-w-[120px] items-center justify-between gap-4 py-0.5 text-[10px]"><span className="flex items-center gap-1.5"><span className="size-1.5 rounded-full" style={{ backgroundColor: item.color }} />{item.name}</span><span className="font-tabular font-semibold">{item.value.toLocaleString("en-IN")}{suffix}</span></div>)}</div>;
}
