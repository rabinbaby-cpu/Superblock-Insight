...
export default function Users() {
  ...
  return (
    <AppShell breadcrumbs={["Users"]}>
      <PageHeader
        eyebrow="User operations"
        title="Users"
        description="Directory of all registered platform users and workspace accounts."
        actions={
          <QuickFormDialog
            type="customer"
            title="Add user"
            description="Create a new user record in the workspace."
            trigger={
              <Button size="sm">
      ...
      <div className="mt-4 panel overflow-hidden">
        <TableSearch
          ...
          placeholder="Search user, contact, or email…"
          actions={
            <>
              <Button
                variant="outline"
              ...
              <Button
                ...
                onClick={() =>
                  toast.success("Filter saved", {
                    description: "User operations · Personal view",
                  })
                }
              >
        ...
        {selected.length > 0 && (
          <div className="flex items-center gap-3 border-b border-primary/20 bg-primary/5 px-3 py-2">
            ...
            <Button
              ...
              onClick={() => toast.success("Owner updated for selected users")}
            >
              Assign owner
            </Button>
            <Button
        ...
        <div className="flex flex-col gap-3 border-t border-border/70 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-[11px] text-muted-foreground">
            {filtered.length === 0
              ? "0 users"
              : `Showing all ${filtered.length} users (scroll to browse)`}
          </div>
          <span className="font-mono text-[10px] text-muted-foreground">
            {filtered.length} of {customers.length} total
          </span>
        </div>
      </div>
    </AppShell>
            ...
            <div>
              ...
              <div className="text-[10px] text-muted-foreground">
                {customer.industry !== "—" || customer.region !== "—"
                  ? `${customer.industry !== "—" ? customer.industry : ""}${customer.industry !== "—" && customer.region !== "—" ? " · " : ""}${customer.region !== "—" ? customer.region : ""}`
                  : "Pending business portfolio configuration"}
              </div>
            </div>
          </Link>
        </td>
      ...
      <td>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={() => toast.success("User edit opened")}>
              Edit user
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => toast.success("Configure business portfolio")}>
              Configure portfolio
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              className="text-rose-600"
              onClick={() => toast.success("User access suspended")}
            >
              Suspend access
            </DropdownMenuItem>
          </DropdownMenuContent>
