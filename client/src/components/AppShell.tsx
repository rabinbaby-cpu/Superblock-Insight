...
function NavContent({ collapsed, onNavigate }: { collapsed: boolean; onNavigate?: () => void }) {
  ...
  return (
    <>
      <div className={cn("flex h-16 items-center border-b border-sidebar-border px-3", collapsed ? "justify-center" : "justify-between")}>
      ...
      <div className="border-t border-sidebar-border p-2">
        <a href="https://superblock.chat/" target="_blank" rel="noreferrer" className={cn("sidebar-link", collapsed && "justify-center px-0")}>
          <HelpCircle className="size-[15px]" />{!collapsed && <span>Help & resources</span>}
        </a>
        {!collapsed && (
...
export function AppShell({ children, breadcrumbs }: { children: ReactNode; breadcrumbs?: string[] }) {
  const { sidebarCollapsed, toggleSidebar, mobileNavOpen, setMobileNavOpen, setSearchOpen, theme, setTheme } = useApp();
  return (
    <div className="min-h-screen bg-background text-foreground">
      <aside className={cn("fixed inset-y-0 left-0 z-40 hidden border-r border-sidebar-border bg-sidebar text-sidebar-foreground transition-[width] duration-200 lg:flex lg:flex-col", sidebarCollapsed ? "w-[64px]" : "w-[220px]")}>
        <NavContent collapsed={sidebarCollapsed} />
        <Button variant="outline" size="icon" className="absolute -right-3 top-20 size-6 rounded-full bg-background shadow-sm" onClick={toggleSidebar} aria-label="Toggle sidebar">
          {sidebarCollapsed ? <ChevronsRight className="size-3" /> : <ChevronsLeft className="size-3" />}
        </Button>
      </aside>

      {mobileNavOpen && <button className="fixed inset-0 z-40 bg-black/30 backdrop-blur-[1px] lg:hidden" onClick={() => setMobileNavOpen(false)} aria-label="Close navigation" />}
      <aside className={cn("fixed inset-y-0 left-0 z-50 flex w-[250px] flex-col border-r bg-sidebar text-sidebar-foreground transition-transform duration-200 lg:hidden", mobileNavOpen ? "translate-x-0" : "-translate-x-full")}>
        <NavContent collapsed={false} onNavigate={() => setMobileNavOpen(false)} />
      </aside>

      <div className={cn("min-h-screen transition-[padding] duration-200", sidebarCollapsed ? "lg:pl-[64px]" : "lg:pl-[220px]")}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild><Link href="/login"><CircleDollarSign className="size-3.5" />Sign out</Link></DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </header>
        <main className="mx-auto w-full max-w-[1660px] p-4 sm:p-5 xl:p-6">{children}</main>
      </div>
      <GlobalSearch />
    </div>
  );
